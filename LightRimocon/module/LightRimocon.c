...
#define DRIVER_NAME "LightRimocon"
...
static int lrimo_exec(int mode){
...
static void lrimo_led(int mode){
	switch(mode){
		case 0://off
			//lighting 0.5sec
			gpio_set_value(LED_PIN, 1);
...
static ssize_t lrimo_write(struct file *file, const char __user *buf, size_t length, loff_t *offset){
	...
	if(lrimo_exec((int)write_val)==0){
		lrimo_led((int)write_val);
	}else{
		lrimo_led(-1);
	}
	return write_length;
...
static int lrimo_init(void){
	...
	if(gpio_direction_input(ON_PIN)){
		printk("gpio%d direction set failed\n",ON_PIN);
		gpio_free(ON_PIN);
		return -1;
	}
